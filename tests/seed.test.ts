...
describe('development seed data', () => {
  ...
  // The acceptance line is about the CLI, so it is tested through the CLI. Asserting on
  // `occupiedTables` alone proves the predicate the branch calls, not that the branch refuses --
  // and the refusal, not the predicate, is what stops a second seed merging two versions of the
  // truth into one database. Runs the emitted artefact under real node, the same standard
  // module-resolution.test.ts sets, so `tsc -b` must have run (it does, in verify and in CI).
  it('refuses a second seed from the CLI itself, and --reset clears the way for one', () => {
    const dir = mkdtempSync(join(tmpdir(), 'seed-cli-'));
    const env = { ...process.env, SEED_DB_PATH: join(dir, 'dev.sqlite3') };
    const run = (...args: string[]) =>
      spawnSync(process.execPath, ['dist/scripts/seed.js', ...args], { env, encoding: 'utf8' });
    try {
      ...
      expect(second.stderr).toMatch(/never upserts/);
